...
app.post('/api/ai/playground-chat', async (req, res) => {
    ...
    const systemPrompt = `Tu es AÏDA, un tuteur pédagogique bienveillant... (etc.)`;
    if (history.length === 0 || history[0].role !== 'system') history.unshift({ role: 'system', content: systemPrompt });
    else history[0].content = systemPrompt;
    try {
