...
const app = express();
// La configuration CORS doit être une des premières choses que l'app utilise.
...
if (!cosmosEndpoint || !cosmosKey) {
    console.error("ERREUR CRITIQUE: Les variables d'environnement COSMOS_ENDPOINT ou COSMOS_KEY ne sont pas définies. Le serveur ne peut pas démarrer.");
}
...
async function cleanUpOcrText(rawText) {
    ...
    } catch (error) { console.error("Erreur lors du nettoyage du texte par l'IA:", error); return rawText; }
}
async function extractTextFromBuffer(buffer) {
    ...
    } catch (error) { console.error("Erreur détaillée de Document Intelligence:", error); throw new Error("L'analyse du document a échoué. Vérifiez le format du fichier et les clés d'API."); }
}
...
app.post('/api/teacher/validate-result', async (req, res) => {
    ...
    if (!classId || !teacherEmail || !studentEmail || !contentId || !appreciation) return res.status(400).json({ error: "Données de validation incomplètes." });
    try {
...
app.post('/api/ai/generate-content', async (req, res) => {
    ...
    const apiKey = process.env.DEEPSEEK_API_KEY;
    if (!apiKey || !competences || !contentType) return res.status(400).json({ error: "Compétences et type de contenu sont requis." });
    const promptMap = {
...
app.post('/api/ai/correct-exercise', async (req, res) => {
    ...
    const apiKey = process.env.DEEPSEEK_API_KEY;
    if (!apiKey || !exerciseText) return res.status(400).json({ error: "Texte de l'exercice requis." });
    const prompt = `Agis comme un tuteur bienveillant. L'exercice est : "${exerciseText}". La réponse de l'élève est : "${studentAnswer}". Corrige sa réponse en expliquant gentiment ses erreurs, sans être décourageant.`;
    try {
        const response = await axios.post('https://api.deepseek.com/chat/completions', { model: 'deepseek-chat', messages: [{ role: 'user', content: prompt }] }, { headers: { 'Authorization': `Bearer ${apiKey}` } });
        res.json({ correction: response.data.choices[0].message.content });
    } catch (error) { res.status(500).json({ error: "Erreur lors de la correction." }); }
});
...
app.post('/api/ai/playground-chat', async (req, res) => {
    ...
    const systemPrompt = `Tu es AÏDA, un tuteur pédagogique IA. Tu as deux modes de fonctionnement en fonction de la demande de l'élève. 1. Mode 'tutor' (Aide aux exercices) : Ton objectif est d'aider l'élève à comprendre. Ne donne JAMAIS la réponse immédiatement. Pose des questions, donne des indices. En dernier recours, donne la solution avec une explication. 2. Mode 'coach' (Préparation de devoir) : Ce mode est strict. Règle d'or : NE JAMAIS DONNER LA RÉPONSE FINALE. Aide sur la méthode, la structure, la reformulation. Si on te demande la réponse, refuse poliment en expliquant ton rôle. Analyse le message de l'utilisateur qui précisera le mode, et adapte ta réponse.`;
